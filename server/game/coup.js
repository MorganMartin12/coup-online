...
class CoupGame{
    ...
    resetGame(startingPlayer = 0) {
        ...
        for(let i = 0; i < this.players.length; i++) {
            ...
            this.players[i].revealed = ['???','???']
        }
    }
    revealCard(challengeeIndex,index,replace) {
        if(replace){
            this.deck.push(this.players[challengeeIndex].influences[index])
            this.deck = gameUtils.shuffleArray(this.deck);
            this.players[challengeeIndex].influences.splice(index,1);
            this.players[challengeeIndex].influences.push(this.deck.pop())
            
        }
        else {
            this.players[challengeeIndex].revealed.shift()
            this.players[challengeeIndex].revealed.push(this.players[challengeeIndex].influences[index])
            this.players[challengeeIndex].influences.splice(index,1);
        }
                ...
                if(bind.isRevealOpen) {
                    ...
                    if(res.isBlock) { //block challenge
                        if(res.revealedCard == res.counterAction.claim || (res.counterAction.counterAction == 'block_steal' && (res.revealedCard == 'ambassador' || res.revealedCard =='captain'))) { //challenge failed
                            ...
                            for(let i = 0; i < bind.players[challengeeIndex].influences.length; i++) { //revealed card needs to be replaced
                                if(bind.players[challengeeIndex].influences[i] == res.revealedCard) {
                                    this.revealCard(challengeeIndex,i,true)
                                    break;
                                }
                            }
                            bind.updatePlayers();
                            ...
                            for(let i = 0; i < bind.players[challengeeIndex].influences.length; i++) {
                                if(bind.players[challengeeIndex].influences[i] == res.revealedCard) {
                                    bind.revealCard(challengeeIndex,i,false)
                                    break;
                                }
                            }
                        ...
                        if(res.revealedCard == bind.actions[res.prevAction.action].influence) { // challenge failed
                            ...
                            for(let i = 0; i < bind.players[challengeeIndex].influences.length; i++) { //revealed card needs to be replaced
                                if(bind.players[challengeeIndex].influences[i] == res.revealedCard) {
                                   bind.revealCard(challengeeIndex,i,true)
                                    break;
                                }
                            }
                            bind.updatePlayers();
